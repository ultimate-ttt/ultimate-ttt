...
function resetGame( game: AnalysisGame ) {
    ...
    game.board.forEach( b => {
        const firstMoveOnThisBoard = arePointsEqual( b.position, firstMove.boardPosition );
        b.value = TileValue.Empty;
        b.tiles.forEach( t => {
            const firstMoveOnThisTile = firstMoveOnThisBoard && arePointsEqual( t.position, firstMove.tilePosition );
